...
CONST_APP_VERSION = "MaxBot (2023.01.12)3版"
...
def get_favoriate_extension_path(webdriver_path):
    no_google_analytics_path = os.path.join(
        webdriver_path, "no_google_analytics_1.1.0.0.crx"
    )
...
def load_chromdriver_normal(webdriver_path, driver_type, adblock_plus_enable):
    ...
    driver = webdriver.Chrome(
        service=chrome_service, options=chrome_options, desired_capabilities=caps
    )
    ...
    if os.path.exists(chromedriver_path):
        ...
        try:
            ...
            if "cannot connect to chrome" in str(exc):
                if "This version of ChromeDriver only supports Chrome version" in str(
                    exc
                ):
        ...
        if is_local_chrome_browser_lower:
            ...
            driver = load_chromdriver_normal(
                webdriver_path, driver_type, adblock_plus_enable
            )
    else:
        print("Oops! web driver not on path:", chromedriver_path)
        print("let uc automatically download chromedriver.")
        driver = uc.Chrome(
            options=options, desired_capabilities=caps, suppress_welcome=False
        )
...
def guess_answer_list_from_multi_options(tmp_text):
    ...
    if options_list is None:
        if "【" in tmp_text and "】" in tmp_text:
            options_list = re.findall("【.*?】", tmp_text)
...
def guess_answer_list_from_symbols(captcha_text_div_text):
    ...
    delimitor_symbols_left = ["(", "[", "{", " ", " ", " ", " "]
    delimitor_symbols_right = [")", "]", "}", ":", ".", ")", "-"]
    idx = -1
    for idx in range(len(delimitor_symbols_left)):
        ...
        if symbol_left in tmp_text and symbol_right in tmp_text and "半形" in tmp_text:
            hint_list = re.findall(
                "\\" + symbol_left + "[\\w]+\\" + symbol_right, tmp_text
            )
...
def guess_answer_list_from_hint(CONST_EXAMPLE_SYMBOL, captcha_text_div_text):
    ...
    if offical_hint_string == "":
        offical_hint_string = get_offical_hint_string_from_symbol(
            CONST_EXAMPLE_SYMBOL, tmp_text
        )
...
def format_question_string(CONST_EXAMPLE_SYMBOL, captcha_text_div_text):
    ...
    for symbol_if in SYMBOL_IF_LIST:
        if symbol_if in tmp_text and "答案" in tmp_text:
            ...
            tmp_text = tmp_text.replace(
                symbol_if + "你答案", CONST_EXAMPLE_SYMBOL + "答案"
            )
            tmp_text = tmp_text.replace(
                symbol_if + "答案", CONST_EXAMPLE_SYMBOL + "答案"
            )
...
def get_answer_list_by_question(CONST_EXAMPLE_SYMBOL, captcha_text_div_text):
    ...
    if return_list is None:
        return_list = guess_answer_list_from_hint(
            CONST_EXAMPLE_SYMBOL, captcha_text_div_text
        )
...
def tixcraft_home(driver):
    ...
    if accept_all_cookies_btn is not None:
        ...
        try:
            if (
                accept_all_cookies_btn.is_enabled()
            ...
            try:
                ...
                try:
                    driver.execute_script(
                        "arguments[0].click();", accept_all_cookies_btn
                    )
                except Exception as exc:
                    pass
    ...
    try:
        close_all_alert_btns = driver.find_elements(
            By.CSS_SELECTOR, "[class='close-alert']"
        )
    except Exception as exc:
...
def tixcraft_date_auto_select(driver, url, config_dict):
    ...
    if check_game_detail:
        try:
            date_list = driver.find_elements(
                By.CSS_SELECTOR, "#gameList > table > tbody > tr"
            )
        except Exception as exc:
            print("find #gameList fail")

    is_coming_soon = False
    coming_soon_condictions_list = [
        "開賣",
        "剩餘",
        "天",
        "小時",
...
def get_tixcraft_target_area(
    el, area_keyword, area_auto_select_mode, pass_1_seat_remaining_enable
):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
...
def tixcraft_auto_ocr(
    driver, ocr, ocr_captcha_with_submit, ocr_captcha_force_submit, previous_answer
):
...
def tixcraft_ticket_main(driver, config_dict, ocr):
    ...
    if not is_ticket_number_assigned:
        ...
        ticket_number = str(config_dict["ticket_number"])
        is_ticket_number_assigned = tixcraft_ticket_number_auto_fill(
        ...
        if is_ticket_number_assigned:
            if not ocr_captcha_enable:
                ...
                for redo_ocr in range(999):
                    is_need_redo_ocr, previous_answer, is_form_sumbited = (
                        tixcraft_auto_ocr(
                            driver,
                            ocr,
                            ocr_captcha_with_submit,
...
def kktix_events_press_next_button(driver):
    ...
    try:
        ...
        next_step_button = wait.until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, "div.tickets a.btn-point"))
        )
    ...
    try:
        ...
        next_step_button = wait.until(
            EC.element_to_be_clickable(
                (
                    By.CSS_SELECTOR,
                    "#registrationsNewApp div.form-actions button.btn-primary",
                )
            )
        )
        if not next_step_button is None:
...
def kktix_input_captcha_text(
    captcha_password_input_tag, inferred_answer_string, force_overwrite=False
):
    show_debug_message = True  # debug.
    # show_debug_message = False      # online
                ...
                if ticket_price_input is not None:
                    ...
                    try:
                        current_ticket_number = str(
                            ticket_price_input.get_attribute("value")
                        ).strip()
                    ...
                    if ticket_number > 1:
                        ...
                        try:
                            span_danger_popup = row.find_element(
                                By.CSS_SELECTOR, "span.danger"
                            )
...
def kktix_assign_ticket_number(
    ...
    is_ticket_number_assigned, areas = kktix_travel_price_list(
        driver,
        ticket_number,
        pass_1_seat_remaining_enable,
        kktix_area_keyword_1,
    ...
    if target_area is not None:
        ...
        try:
            ...
            ticket_price_input = target_area.find_element(
                By.CSS_SELECTOR, "input[type='text']"
            )
        except Exception as exc:
            pass
...
def kktix_get_web_datetime(registrationsNewApp_div):
    ...
    if not registrationsNewApp_div is None:
        try:
            el_web_datetime_list = registrationsNewApp_div.find_elements(
                By.TAG_NAME, "td"
            )
        except Exception as exc:
            if show_debug_message:
    ...
    if is_match_event_code:
        ...
        try:
            html_result = requests.get(
                url, headers=headers, timeout=0.7, allow_redirects=False
            )
        except Exception as exc:
            html_result = None
...
def get_answer_string_from_web_date(
    CONST_EXAMPLE_SYMBOL, registrationsNewApp_div, captcha_text_div_text
):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    if is_need_parse_web_datetime:
        ...
        if not web_datetime is None:
            ...
            captcha_text_formatted = format_question_string(
                CONST_EXAMPLE_SYMBOL, captcha_text_div_text
            )
            if show_debug_message:
            ...
            if my_datetime_foramted is None:
                if CONST_EXAMPLE_SYMBOL in captcha_text_formatted:
                    ...
                    my_anwser_formated = convert_string_to_pattern(
                        number_text, dynamic_length=False
                    )
                ...
                for guess_year in range(now.year - 4, now.year + 2):
                    ...
                    if current_year in captcha_text_formatted:
                        ...
                        if my_delimitor_symbol in my_hint_anwser:
                            my_delimitor_index = my_hint_anwser.find(
                                my_delimitor_symbol
                            )
                            my_hint_anwser = my_hint_anwser[
                                my_delimitor_index + len(my_delimitor_symbol) :
                            ]
                        # print("my_hint_anwser:", my_hint_anwser)
                        # get before.
                        my_delimitor_symbol = "，"
                        ...
                        my_anwser_formated = convert_string_to_pattern(
                            my_hint_anwser, dynamic_length=False
                        )
                        if (
                            my_anwser_formated
                            == "[\\d][\\d][\\d][\\d][\\d][\\d][\\d][\\d]"
            ...
            if not my_datetime_foramted is None:
                ...
                if my_delimitor_symbol in web_datetime:
                    web_datetime = web_datetime[
                        : web_datetime.find(my_delimitor_symbol)
                    ]
...
def get_answer_string_from_web_time(
    CONST_EXAMPLE_SYMBOL, registrationsNewApp_div, captcha_text_div_text
):
    show_debug_message = True  # debug.
    # show_debug_message = False      # online
    ...
    if is_need_parse_web_time:
        ...
        if not registrationsNewApp_div is None:
            web_datetime = kktix_get_web_datetime(registrationsNewApp_div)
        if not web_datetime is None:
            tmp_text = format_question_string(
                CONST_EXAMPLE_SYMBOL, captcha_text_div_text
            )
            ...
            if not my_datetime_foramted is None:
                ...
                if date_delimitor_symbol in web_datetime:
                    date_delimitor_symbol_index = web_datetime.find(
                        date_delimitor_symbol
                    )
...
def get_answer_list_from_question_string(
    ...
    if inferred_answer_string is None:
        ...
        if "「" in captcha_text_div_text and "」" in captcha_text_div_text:
            if (
                "下" in captcha_text_div_text
                and "空" in captcha_text_div_text
                and "輸入" in captcha_text_div_text
                and "引號" in captcha_text_div_text
                and "字" in captcha_text_div_text
            ):
                is_use_quota_message = True
            if (
                "半形" in captcha_text_div_text
                and "輸入" in captcha_text_div_text
                and "引號" in captcha_text_div_text
                and "字" in captcha_text_div_text
            ):
    ...
    # parse '演出日期'
    if inferred_answer_string is None:
        inferred_answer_string = get_answer_string_from_web_date(
            CONST_EXAMPLE_SYMBOL, registrationsNewApp_div, captcha_text_div_text
        )

    # parse '演出時間'
    if inferred_answer_string is None:
        inferred_answer_string = get_answer_string_from_web_time(
            CONST_EXAMPLE_SYMBOL, registrationsNewApp_div, captcha_text_div_text
        )
    ...
    if inferred_answer_string is None:
        if "name of event" in captcha_text_div_text:
            if (
                "(" in captcha_text_div_text
                and ")" in captcha_text_div_text
                and "ans:" in captcha_text_div_text.lower()
            ):
                target_symbol = "("
                ...
                inferred_answer_string = captcha_text_div_text[
                    star_index + 1 : end_index
                ]
                # print("inferred_answer_string:", inferred_answer_string)
    ...
    if inferred_answer_string is None:
        if not is_combine_two_question:
            answer_list = get_answer_list_by_question(
                CONST_EXAMPLE_SYMBOL, captcha_text_div_text
            )
            if show_debug_message:
...
def kktix_reg_new_captcha(registrationsNewApp_div, captcha_inner_div):
    ...
    if not captcha_text_div_text is None:
        ...
        inferred_answer_string, answer_list = get_answer_list_from_question_string(
            registrationsNewApp_div, captcha_text_div_text
        )
...
def kktix_reg_new_main(driver, answer_index, is_finish_checkbox_click, config_dict):
    ...
    if not registrationsNewApp_div is None:
        ...
        for retry_index in range(2):
            is_ticket_number_assigned = kktix_assign_ticket_number(
                driver,
                ticket_number,
                pass_1_seat_remaining_enable,
                kktix_area_auto_select_mode,
    ...
    if is_ticket_number_assigned:
        try:
            captcha_inner_div = driver.find_element(
                By.CSS_SELECTOR, ".custom-captcha-inner"
            )
        except Exception as exc:
            pass
    ...
    if not captcha_inner_div is None:
        try:
            captcha_password_input_tag = captcha_inner_div.find_element(
                By.TAG_NAME, "input"
            )
            if show_debug_message:
    ...
    if not captcha_password_input_tag is None:
        ...
        if captcha_inner_div is not None:
            ...
            if len(user_guess_string) > 0:
                ...
                if auto_guess_options:
                    inferred_answer_string, answer_list = kktix_reg_new_captcha(
                        registrationsNewApp_div, captcha_inner_div
                    )

            if inferred_answer_string is not None:
                # password is not None, try to send.
                is_cpatcha_sent = kktix_input_captcha_text(
                    captcha_password_input_tag, inferred_answer_string
                )
                if is_cpatcha_sent:
                ...
                if is_try_to_focus:
                    ...
                    inputed_captcha_text = None
                    try:
                        inputed_captcha_text = captcha_password_input_tag.get_attribute(
                            "value"
                        )
                    except Exception as exc:
                        pass
        ...
        if show_debug_message:
            ...
            print(
                "is_captcha_appear_and_filled_password:",
                is_captcha_appear_and_filled_password,
            )
    ...
    is_do_press_next_button = False
    if is_ticket_number_assigned:
        auto_press_next_step_button = config_dict["kktix"][
            "auto_press_next_step_button"
        ]
        if auto_press_next_step_button:
            if is_finish_checkbox_click:
                is_do_press_next_button = kktix_double_check_all_text_value(
                    driver, ticket_number_str
                )
            else:
                        ...
                        if answer_index < len(answer_list) - 1:
                            if kktix_captcha_text_value(captcha_inner_div) == "":
                                ...
                                if len(answer_string) > 0:
                                    ...
                                    is_cpatcha_sent = kktix_input_captcha_text(
                                        captcha_password_input_tag, answer_string
                                    )
                                    if is_cpatcha_sent:
...
def kktix_reg_new(driver, url, answer_index, kktix_register_status_last, config_dict):
    ...
    if is_need_refresh:
        ...
        if auto_fill_ticket_number:
            answer_index = kktix_reg_new_main(
                driver, answer_index, is_finish_checkbox_click, config_dict
            )
...
def get_fami_target_area(
    ...
    if not formated_area_list is None:
        if len(formated_area_list) > 0:
            ...
            if (
                len(date_keyword) == 0
                and len(area_keyword_1) == 0
                and len(area_keyword_2) == 0
            ):
                # select all.
    ...
    if is_select_box_visible:
        ...
        if ticket_number_select is not None:
            try:
                ...
                if (
                    ticket_number_select.first_selected_option.text == "0"
                    or ticket_number_select.first_selected_option.text == "選擇張數"
                ):
                    # target ticket number
...
def urbtix_date_auto_select(
    driver, auto_select_mode, date_keyword, auto_reload_coming_soon_page_enable
):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
...
def urbtix_area_auto_select(
    driver, area_auto_select_mode, area_keyword_1, area_keyword_1_and
):
    show_debug_message = True  # debug.
    show_debug_message = False  # online
    ...
    if auto_fill_ticket_number:
        ...
        if not is_need_refresh:
            if not is_price_assign_by_bot:
                if len(area_keyword_3) > 0:
                    is_need_refresh, is_price_assign_by_bot = urbtix_area_auto_select(
                        driver, area_auto_select_mode, area_keyword_3, ""
                    )
        ...
        if not is_need_refresh:
            if not is_price_assign_by_bot:
                if len(area_keyword_4) > 0:
                    is_need_refresh, is_price_assign_by_bot = urbtix_area_auto_select(
                        driver, area_auto_select_mode, area_keyword_4, ""
                    )
...
def cityline_ticket_number_auto_select(driver, ticket_number):
    ...
    return assign_ticket_number_by_select(
        driver, ticket_number, by_method, selector_string
    )
...
def ibon_activity_info(driver, config_dict):
    ...
    if show_debug_message:
        print("date_keyword:", date_keyword)
        print(
            "auto_reload_coming_soon_page_enable:", auto_reload_coming_soon_page_enable
        )
...
def kktix_main(driver, url, config_dict, answer_index, kktix_register_status_last):
    ...
    if not is_url_contain_sign_in:
        if "/registrations/new" in url:
            answer_index, kktix_register_status_last = kktix_reg_new(
                driver, url, answer_index, kktix_register_status_last, config_dict
            )
        else:
            is_event_page = False
            if "/events/" in url:
...
def cityline_main(driver, url, config_dict):
    ...
    if "/eventDetail?" in url:
        ...
        if is_modal_dialog_popup:
            ...
            date_auto_select_enable = config_dict["tixcraft"]["date_auto_select"][
                "enable"
            ]
            if date_auto_select_enable:
    ...
    if "/performance?" in url:
        ...
        if is_modal_dialog_popup:
            ...
            area_auto_select_enable = config_dict["tixcraft"]["area_auto_select"][
                "enable"
            ]
            if area_auto_select_enable:
...
def main():
    ...
    DISCONNECTED_MSG = (
        "Unable to evaluate script: no such window: target window already closed"
    )
    ...
    while True:
        ...
        if tixcraft_family:
            is_verifyCode_editing = tixcraft_main(
                driver, url, config_dict, is_verifyCode_editing, ocr
            )

        # for kktix.cc and kktix.com
        if "kktix.c" in url:
            answer_index, kktix_register_status_last = kktix_main(
                driver, url, config_dict, answer_index, kktix_register_status_last
            )
