...
CONST_APP_VERSION = "MaxBot (2023.01.13)"
...
def load_translate():
    ...
    en_us["maxbot_slogan"] = (
        "MaxBot is a FREE and open source bot program. Wish you good luck."
    )
    en_us["donate"] = "Donate"
    en_us["help"] = "Help"
    ...
    ja_jp["maxbot_slogan"] = (
        "MaxBot は無料のオープン ソース ボット プログラムです。チケットの成功をお祈りします。"
    )
    ja_jp["donate"] = "寄付"
    ja_jp["help"] = "利用方法"
    ja_jp["release"] = "リリース"
...
def btn_save_act(language_code, slience_mode=False):
    ...
    if is_all_data_correct:
        config_dict["pass_1_seat_remaining"] = bool(
            chk_state_pass_1_seat_remaining.get()
        )
        ...
        config_dict["kktix"][
            "area_keyword_1_and"
        ] = txt_kktix_area_keyword_1_and.get().strip()
        config_dict["kktix"]["area_keyword_2"] = txt_kktix_area_keyword_2.get().strip()
        config_dict["kktix"][
            "area_keyword_2_and"
        ] = txt_kktix_area_keyword_2_and.get().strip()
        # disable password brute force attack
        # config_dict["kktix"]["answer_dictionary"] = txt_kktix_answer_dictionary.get().strip()
        config_dict["kktix"]["auto_guess_options"] = bool(
            chk_state_auto_guess_options.get()
        )
        config_dict["kktix"][
            "user_guess_string"
        ] = txt_kktix_user_guess_string.get().strip()

        config_dict["tixcraft"]["date_auto_select"]["enable"] = bool(
            chk_state_date_auto_select.get()
        )
        config_dict["tixcraft"]["date_auto_select"][
            "date_keyword"
        ] = txt_date_keyword.get().strip()

        config_dict["tixcraft"]["area_auto_select"]["enable"] = bool(
            chk_state_area_auto_select.get()
        )
        config_dict["tixcraft"]["area_auto_select"][
            "area_keyword_1"
        ] = txt_area_keyword_1.get().strip()
        config_dict["tixcraft"]["area_auto_select"][
            "area_keyword_2"
        ] = txt_area_keyword_2.get().strip()
        config_dict["tixcraft"]["area_auto_select"][
            "area_keyword_3"
        ] = txt_area_keyword_3.get().strip()
        config_dict["tixcraft"]["area_auto_select"][
            "area_keyword_4"
        ] = txt_area_keyword_4.get().strip()

        config_dict["tixcraft"]["date_auto_select"][
            "mode"
        ] = combo_date_auto_select_mode.get().strip()
        config_dict["tixcraft"]["area_auto_select"][
            "mode"
        ] = combo_area_auto_select_mode.get().strip()
        ...
        config_dict["advanced"]["play_captcha_sound"]["enable"] = bool(
            chk_state_play_captcha_sound.get()
        )
        config_dict["advanced"]["play_captcha_sound"][
            "filename"
        ] = txt_captcha_sound_filename.get().strip()
        ...
        config_dict["advanced"]["adblock_plus_enable"] = bool(
            chk_state_adblock_plus.get()
        )

        config_dict["ocr_captcha"] = {}
        config_dict["ocr_captcha"]["enable"] = bool(chk_state_ocr_captcha.get())
        config_dict["ocr_captcha"]["auto_submit"] = bool(
            chk_state_ocr_captcha_with_submit.get()
        )
    ...
    if is_all_data_correct:
        ...
        if not slience_mode:
            messagebox.showinfo(
                translate[language_code]["save"], translate[language_code]["done"]
            )
    ...
    if save_ret:
        if hasattr(sys, "frozen"):
            ...
            try:
                ...
                s = subprocess.Popen(
                    [interpreter_binary, "chrome_tixcraft.py"], cwd=working_dir
                )
                ...
                try:
                    s = subprocess.Popen(
                        [interpreter_binary_alt, "chrome_tixcraft.py"], cwd=working_dir
                    )
                except Exception as exc:
                    msg = str(exc)
                    print("exeption:", msg)
...
def applyNewLanguage():
    ...
    lbl_pass_1_seat_remaining.config(
        text=translate[language_code]["pass_1_seat_remaining"]
    )
    ...
    lbl_ocr_captcha_with_submit.config(
        text=translate[language_code]["ocr_captcha_with_submit"]
    )
    ...
    lbl_captcha_sound_filename.config(
        text=translate[language_code]["captcha_sound_filename"]
    )
    ...
    lbl_adblock_plus_settings.config(
        text=translate[language_code]["adblock_plus_settings"]
    )
...
def showHideOcrCaptchaWithSubmit():
    ...
    if is_ocr_captcha_enable:
        # show.
        lbl_ocr_captcha_with_submit.grid(
            column=0, row=ocr_captcha_with_submit_index, sticky=E
        )
        chk_ocr_captcha_with_sumit.grid(
            column=1, row=ocr_captcha_with_submit_index, sticky=W
        )
    else:
        # hide
    ...
    if is_ocr_captcha_auto_submit_enable:
        ...
        chk_ocr_captcha_force_sumit.grid(
...
def showHidePass1SeatRemaining():
    ...
    if ticket_number_int > 1:
        # show.
        lbl_pass_1_seat_remaining.grid(
            column=0, row=pass_1_seat_remaining_index, sticky=E
        )
        chk_pass_1_seat_remaining.grid(
            column=1, row=pass_1_seat_remaining_index, sticky=W
        )
    else:
        # hide
...
def PreferenctTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    area_keyword_1 = config_dict["tixcraft"]["area_auto_select"][
        "area_keyword_1"
    ].strip()
    area_keyword_2 = config_dict["tixcraft"]["area_auto_select"][
        "area_keyword_2"
    ].strip()
    area_keyword_3 = config_dict["tixcraft"]["area_auto_select"][
        "area_keyword_3"
    ].strip()
    area_keyword_4 = config_dict["tixcraft"]["area_auto_select"][
        "area_keyword_4"
    ].strip()
    ...
    global lbl_pass_1_seat_remaining
    lbl_pass_1_seat_remaining = Label(
        frame_group_header, text=translate[language_code]["pass_1_seat_remaining"]
    )
    ...
    global chk_pass_1_seat_remaining
    chk_pass_1_seat_remaining = Checkbutton(
        frame_group_header,
        text=translate[language_code]["enable"],
        variable=chk_state_pass_1_seat_remaining,
    )
    ...
    global lbl_auto_check_agree
    lbl_auto_check_agree = Label(
        frame_group_header, text=translate[language_code]["auto_check_agree"]
    )
    ...
    global chk_auto_check_agree
    chk_auto_check_agree = Checkbutton(
        frame_group_header,
        text=translate[language_code]["enable"],
        variable=chk_state_auto_check_agree,
    )
    ...
    global lbl_kktix_area_mode
    lbl_kktix_area_mode = Label(
        frame_group_kktix, text=translate[language_code]["area_select_order"]
    )
    ...
    global lbl_kktix_area_keyword_1
    lbl_kktix_area_keyword_1 = Label(
        frame_group_kktix, text=translate[language_code]["area_keyword_1"]
    )
    lbl_kktix_area_keyword_1.grid(column=0, row=group_row_count, sticky=E)

    global txt_kktix_area_keyword_1
    txt_kktix_area_keyword_1_value = StringVar(
        frame_group_kktix, value=kktix_area_keyword_1
    )
    txt_kktix_area_keyword_1 = Entry(
        frame_group_kktix, width=20, textvariable=txt_kktix_area_keyword_1_value
    )
    ...
    global lbl_kktix_area_keyword_1_and_text
    lbl_kktix_area_keyword_1_and_text = Label(
        frame_group_kktix, text=translate[language_code]["and"]
    )
    ...
    global txt_kktix_area_keyword_1_and
    txt_kktix_area_keyword_1_and_value = StringVar(
        frame_group_kktix, value=kktix_area_keyword_1_and
    )
    txt_kktix_area_keyword_1_and = Entry(
        frame_group_kktix, width=20, textvariable=txt_kktix_area_keyword_1_and_value
    )
    ...
    global lbl_kktix_area_keyword_2
    lbl_kktix_area_keyword_2 = Label(
        frame_group_kktix, text=translate[language_code]["area_keyword_2"]
    )
    lbl_kktix_area_keyword_2.grid(column=0, row=group_row_count, sticky=E)

    global txt_kktix_area_keyword_2
    txt_kktix_area_keyword_2_value = StringVar(
        frame_group_kktix, value=kktix_area_keyword_2
    )
    txt_kktix_area_keyword_2 = Entry(
        frame_group_kktix, width=20, textvariable=txt_kktix_area_keyword_2_value
    )
    ...
    global lbl_kktix_area_keyword_2_and_text
    lbl_kktix_area_keyword_2_and_text = Label(
        frame_group_kktix, text=translate[language_code]["and"]
    )
    ...
    global txt_kktix_area_keyword_2_and
    txt_kktix_area_keyword_2_and_value = StringVar(
        frame_group_kktix, value=kktix_area_keyword_2_and
    )
    txt_kktix_area_keyword_2_and = Entry(
        frame_group_kktix, width=20, textvariable=txt_kktix_area_keyword_2_and_value
    )
    ...
    lbl_auto_guess_options = Label(
        frame_group_kktix, text=translate[language_code]["auto_guess_options"]
    )
    ...
    lbl_user_guess_string = Label(
        frame_group_kktix, text=translate[language_code]["user_guess_string"]
    )
    lbl_user_guess_string.grid(column=0, row=group_row_count, sticky=E)

    global txt_kktix_user_guess_string
    txt_kktix_user_guess_string_value = StringVar(
        frame_group_kktix, value=user_guess_string
    )
    txt_kktix_user_guess_string = Entry(
        frame_group_kktix, width=20, textvariable=txt_kktix_user_guess_string_value
    )
    ...
    chk_date_auto_select = Checkbutton(
        frame_group_tixcraft,
        text=translate[language_code]["enable"],
        variable=chk_state_date_auto_select,
    ...
    combo_date_auto_select_mode["values"] = (
        CONST_FROM_TOP_TO_BOTTOM,
        CONST_FROM_BOTTOM_TO_TOP,
    )
    ...
    global lbl_date_keyword
    lbl_date_keyword = Label(
        frame_group_tixcraft, text=translate[language_code]["date_keyword"]
    )
    lbl_date_keyword.grid(column=0, row=date_keyword_index, sticky=E)

    global txt_date_keyword
    txt_date_keyword_value = StringVar(frame_group_tixcraft, value=date_keyword)
    txt_date_keyword = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_date_keyword_value
    )
    txt_date_keyword.grid(column=1, row=date_keyword_index, sticky=W)
    ...
    lbl_area_auto_select = Label(
        frame_group_tixcraft, text=translate[language_code]["area_auto_select"]
    )
    ...
    chk_area_auto_select = Checkbutton(
        frame_group_tixcraft,
        text=translate[language_code]["enable"],
        variable=chk_state_area_auto_select,
        command=callbackDateAutoOnChange,
    )
    ...
    global lbl_area_keyword_1
    lbl_area_keyword_1 = Label(
        frame_group_tixcraft, text=translate[language_code]["area_keyword_1"]
    )
    lbl_area_keyword_1.grid(column=0, row=area_keyword_1_index, sticky=E)

    global txt_area_keyword_1
    txt_area_keyword_1_value = StringVar(frame_group_tixcraft, value=area_keyword_1)
    txt_area_keyword_1 = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_area_keyword_1_value
    )
    txt_area_keyword_1.grid(column=1, row=area_keyword_1_index, sticky=W)
    ...
    global lbl_area_keyword_2
    lbl_area_keyword_2 = Label(
        frame_group_tixcraft, text=translate[language_code]["area_keyword_2"]
    )
    lbl_area_keyword_2.grid(column=0, row=area_keyword_2_index, sticky=E)
    ...
    txt_area_keyword_2 = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_area_keyword_2_value
    )
    txt_area_keyword_2.grid(column=1, row=area_keyword_2_index, sticky=W)
    ...
    global lbl_area_keyword_3
    lbl_area_keyword_3 = Label(
        frame_group_tixcraft, text=translate[language_code]["area_keyword_3"]
    )
    lbl_area_keyword_3.grid(column=0, row=area_keyword_3_index, sticky=E)

    global txt_area_keyword_3
    txt_area_keyword_3_value = StringVar(frame_group_tixcraft, value=area_keyword_3)
    txt_area_keyword_3 = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_area_keyword_3_value
    )
    txt_area_keyword_3.grid(column=1, row=area_keyword_3_index, sticky=W)
    ...
    global lbl_area_keyword_4
    lbl_area_keyword_4 = Label(
        frame_group_tixcraft, text=translate[language_code]["area_keyword_4"]
    )
    lbl_area_keyword_4.grid(column=0, row=area_keyword_4_index, sticky=E)

    global txt_area_keyword_4
    txt_area_keyword_4_value = StringVar(frame_group_tixcraft, value=area_keyword_4)
    txt_area_keyword_4 = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_area_keyword_4_value
    )
    txt_area_keyword_4.grid(column=1, row=area_keyword_4_index, sticky=W)
    ...
    global lbl_presale_code
    lbl_presale_code = Label(
        frame_group_tixcraft, text=translate[language_code]["user_guess_string"]
    )
    ...
    txt_presale_code = Entry(
        frame_group_tixcraft, width=20, textvariable=txt_presale_code_value
    )
    ...
    lbl_ocr_captcha = Label(
        frame_group_tixcraft, text=translate[language_code]["ocr_captcha"]
    )
    ...
    chk_ocr_captcha = Checkbutton(
        frame_group_tixcraft,
        text=translate[language_code]["enable"],
        variable=chk_state_ocr_captcha,
    ...
    global lbl_ocr_captcha_with_submit
    lbl_ocr_captcha_with_submit = Label(
        frame_group_tixcraft, text=translate[language_code]["ocr_captcha_with_submit"]
    )
    lbl_ocr_captcha_with_submit.grid(
        column=0, row=ocr_captcha_with_submit_index, sticky=E
    )
    ...
    global chk_ocr_captcha_with_sumit
    chk_ocr_captcha_with_sumit = Checkbutton(
        frame_group_tixcraft,
        text=translate[language_code]["enable"],
        variable=chk_state_ocr_captcha_with_submit,
        command=showHideOcrCaptchaWithSubmit,
    )
    ...
    lbl_ocr_captcha_force_submit = Label(
        frame_group_tixcraft, text=translate[language_code]["ocr_captcha_force_submit"]
    )
    lbl_ocr_captcha_force_submit.grid(
        column=0, row=ocr_captcha_force_submit_index, sticky=E
    )
    ...
    global chk_ocr_captcha_force_sumit
    chk_ocr_captcha_force_sumit = Checkbutton(
        frame_group_tixcraft,
        text=translate[language_code]["enable"],
...
def AdvancedTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    if "advanced" in config_dict:
        ...
        if "play_captcha_sound" in config_dict["advanced"]:
            if "enable" in config_dict["advanced"]["play_captcha_sound"]:
                play_captcha_sound = config_dict["advanced"]["play_captcha_sound"][
                    "enable"
                ]
            if "filename" in config_dict["advanced"]["play_captcha_sound"]:
                captcha_sound_filename = config_dict["advanced"]["play_captcha_sound"][
                    "filename"
                ].strip()
    ...
    txt_facebook_account = Entry(
        frame_group_header, width=20, textvariable=txt_facebook_account_value
    )
    ...
    txt_kktix_account = Entry(
        frame_group_header, width=20, textvariable=txt_kktix_account_value
    )
    ...
    global lbl_play_captcha_sound
    lbl_play_captcha_sound = Label(
        frame_group_header, text=translate[language_code]["play_captcha_sound"]
    )
    ...
    global chk_play_captcha_sound
    chk_play_captcha_sound = Checkbutton(
        frame_group_header,
        text=translate[language_code]["enable"],
        variable=chk_state_play_captcha_sound,
    )
    ...
    global lbl_captcha_sound_filename
    lbl_captcha_sound_filename = Label(
        frame_group_header, text=translate[language_code]["captcha_sound_filename"]
    )
    lbl_captcha_sound_filename.grid(column=0, row=group_row_count, sticky=E)

    # print("captcha_sound_filename:", captcha_sound_filename)
    global txt_captcha_sound_filename
    txt_captcha_sound_filename_value = StringVar(
        frame_group_header, value=captcha_sound_filename
    )
    txt_captcha_sound_filename = Entry(
        frame_group_header, width=20, textvariable=txt_captcha_sound_filename_value
    )
    ...
    global lbl_adblock_plus
    lbl_adblock_plus = Label(
        frame_group_header, text=translate[language_code]["adblock_plus_enable"]
    )
    ...
    global chk_adblock_plus
    chk_adblock_plus = Checkbutton(
        frame_group_header,
        text=translate[language_code]["enable"],
        variable=chk_state_adblock_plus,
    )
    ...
    global lbl_adblock_plus_memo
    lbl_adblock_plus_memo = Label(
        frame_group_header, text=translate[language_code]["adblock_plus_memo"]
    )
    ...
    global lbl_adblock_plus_settings
    lbl_adblock_plus_settings = Label(
        frame_group_header, text=translate[language_code]["adblock_plus_settings"]
    )
...
def AboutTab(root, language_code):
    ...
    lbl_slogan = Label(
        frame_group_header,
        text=translate[language_code]["maxbot_slogan"],
        wraplength=400,
        justify="center",
    )
    ...
    lbl_donate_url = Label(
        frame_group_header, text=URL_DONATE, fg="blue", cursor="hand2"
    )
    ...
    lbl_release_url = Label(
        frame_group_header, text=URL_RELEASE, fg="blue", cursor="hand2"
    )
    ...
    lbl_fb_fans = Label(frame_group_header, text="Facebook")
...
def get_action_bar(root, language_code):
    ...
    btn_run = ttk.Button(
        frame_action,
        text=translate[language_code]["run"],
        command=lambda: btn_run_clicked(language_code),
    )
    btn_run.grid(column=0, row=0)

    btn_save = ttk.Button(
        frame_action,
        text=translate[language_code]["save"],
        command=lambda: btn_save_clicked(language_code),
    )
